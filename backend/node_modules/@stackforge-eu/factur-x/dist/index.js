...
import { AFRelationship, PDFDocument, PDFName, PDFArray, PDFDict, PDFStream, PDFRawStream, decodePDFRawStream, PDFString, PDFHexString, PDFRef } from 'pdf-lib';
...
var VatCategoryCode = /* @__PURE__ */ ((VatCategoryCode2) => {
  VatCategoryCode2["STANDARD_RATE"] = "S";
  VatCategoryCode2["ZERO_RATED"] = "Z";
  VatCategoryCode2["EXEMPT"] = "E";
  VatCategoryCode2["REVERSE_CHARGE"] = "AE";
  VatCategoryCode2["INTRA_COMMUNITY_SUPPLY"] = "K";
  VatCategoryCode2["FREE_EXPORT"] = "G";
  VatCategoryCode2["OUTSIDE_SCOPE"] = "O";
  VatCategoryCode2["CANARY_ISLANDS_TAX"] = "L";
  VatCategoryCode2["CEUTA_MELILLA_TAX"] = "M";
  return VatCategoryCode2;
})(VatCategoryCode || {});
...
var XRECHNUNG_PROFILE_URN = "urn:cen.eu:en16931:2017#compliant#urn:xeinkauf.de:kosit:xrechnung_3.0";
  ...
  if (p.legalOrganization) {
    ...
    if (p.legalOrganization.id) {
      lo += tag(
        ...
        p.legalOrganization.schemeID ? { schemeID: p.legalOrganization.schemeID } : void 0
      );
    }
...
function buildLineItem(line, profile) {
  ...
  if (line.manufacturer && atLeast(profile, "EXTENDED" /* EXTENDED */))
  ...
  const basisQuantity = line.basisQuantity !== void 0 ? tag("ram:BasisQuantity", line.basisQuantity.toString(), {
    unitCode: line.basisQuantityUnitCode ?? unitCode
  }) : "";
  if (line.priceDiscount !== void 0 && line.grossUnitPrice === void 0) {
    throw new Error(
      `buildXml: line "${line.id}" sets "priceDiscount" (BT-147) without "grossUnitPrice" (BT-148).`
    );
  }
  if (line.grossUnitPrice !== void 0) {
    const discount = line.priceDiscount !== void 0 ? tag(
      "ram:AppliedTradeAllowanceCharge",
      tag("ram:ChargeIndicator", tag("udt:Indicator", "false")) + tag("ram:ActualAmount", fmtPrice(line.priceDiscount))
    ) : "";
    agree += tag(
...
function buildAllowanceCharge(ac, profile) {
  ...
  if (ac.vatCategoryCode || ac.vatRatePercent !== void 0) {
    const extended = atLeast(profile, "EXTENDED" /* EXTENDED */);
    let tx = tag("ram:TypeCode", "VAT");
...
function buildMonetarySummation(t, profile) {
  ...
  if (atLeast(profile, "EN16931" /* EN16931 */) && t.roundingAmount !== void 0)
...
function buildXml(input, profile, flavor) {
  ...
  const profileUrn = fl === "xrechnung" /* XRECHNUNG */ ? XRECHNUNG_PROFILE_URN : PROFILE_URNS[profile];
  ...
  if (atLeast(profile, "BASIC" /* BASIC */) && !hasActualDelivery && typeCode !== "386" /* PREPAYMENT_INVOICE */) {
    del += tag(
      "ram:ActualDeliverySupplyChainEvent",
  ...
  if (payment?.meansCode && atLeast(profile, "BASIC_WL" /* BASIC_WL */)) {
    ...
    if (payment.debtorIban) {
      ...
      if (payment.debtorAccountName && atLeast(profile, "EXTENDED" /* EXTENDED */))
    ...
    if (payment.debtorBic && atLeast(profile, "EXTENDED" /* EXTENDED */))
      pm += tag(
  ...
  if (input.financialAdjustments && atLeast(profile, "EXTENDED" /* EXTENDED */)) {
...
function validateInput(input, profile, flavor) {
  const errors = [];
  ...
  if (req("BASIC" /* BASIC */) && input.lines) {
    input.lines.forEach((line, i) => {
  ...
  if (req("BASIC_WL" /* BASIC_WL */) && input.totals) {
  ...
  if (req("BASIC" /* BASIC */) && input.totals && input.lines) {
    const sumLineNets = input.lines.reduce((sum, l) => {
      const lineNet = l.lineTotal ?? (typeof l.quantity === "number" && typeof l.unitPrice === "number" ? l.quantity * l.unitPrice : 0);
      return sum + lineNet;
    }, 0);
    if (input.totals.lineTotal != null) {
  ...
  if (req("BASIC_WL" /* BASIC_WL */) && input.totals?.taxTotal != null && input.vatBreakdown) {
  ...
  if (req("BASIC_WL" /* BASIC_WL */) && input.vatBreakdown) {
      ...
      if (vb.taxableAmount != null && vb.taxAmount != null && vb.ratePercent != null) {
        ...
        if (Math.abs(round2(vb.taxAmount) - expected) > 0.01) {
          addError(
            errors,
            `vatBreakdown[${i}].taxAmount`,
            `taxAmount ${vb.taxAmount} does not equal round(taxableAmount \xD7 ratePercent / 100) = ${expected} (BR-CO-17).`,
            "BASIC_WL" /* BASIC_WL */
          );
        }
      }
  ...
  if (req("BASIC" /* BASIC */) && input.document) {
...
async function validateXsd(xml, profile, options) {
  ...
  const schemaFileFor = (requested) => {
    ...
    return files.includes(name) ? name : void 0;
  };
  xmlCleanupInputProvider();
  xmlRegisterInputProvider({
    match: (filename) => schemaFileFor(filename) !== void 0,
...
var XMP_CONFORMANCE_LEVEL = {
  ["MINIMUM" /* MINIMUM */]: "MINIMUM",
  ["BASIC_WL" /* BASIC_WL */]: "BASIC WL",
  ["BASIC" /* BASIC */]: "BASIC",
  ["EN16931" /* EN16931 */]: "EN 16931",
  ["EXTENDED" /* EXTENDED */]: "EXTENDED"
};
function resolveXmpConformanceLevel(profile, flavor) {
  if (flavor === "xrechnung" /* XRECHNUNG */) return "XRECHNUNG";
  return XMP_CONFORMANCE_LEVEL[profile];
}
function ensureIdTrailer(pdfDoc, pdfContentBytes) {
...
function ensureOutputIntent(pdfDoc, iccBytes, identifier) {
...
function findUnembeddedFonts(pdfDoc) {
  const seen = /* @__PURE__ */ new Set();
  const unembedded = /* @__PURE__ */ new Set();
  const FONT_FILE_KEYS = [PDFName.of("FontFile"), PDFName.of("FontFile2"), PDFName.of("FontFile3")];
  const inspectFontDict = (font) => {
  ...
  for (const page of pdfDoc.getPages()) {
    ...
    for (const [, value] of fonts.entries()) {
      const ref = value instanceof PDFRef ? value : void 0;
      if (ref) {
...
` + xsdValidation.errors.map((e) => `  - ${e.message}`).join("\n")
  ...
  if (addPdfA3Metadata) {
    if (rgbIccProfile) {
      const iccBytes = rgbIccProfile instanceof Uint8Array ? rgbIccProfile : new Uint8Array(rgbIccProfile);
...
function detectProfile(xml) {
  const profileMap = [
    ["urn:cen.eu:en16931:2017#conformant#urn:factur-x.eu:1p0:extended", "EXTENDED" /* EXTENDED */],
    ["urn:cen.eu:en16931:2017#conformant#urn:zugferd.de:2p0:extended", "EXTENDED" /* EXTENDED */],
    ["urn:cen.eu:en16931:2017#compliant#urn:factur-x.eu:1p0:basic", "BASIC" /* BASIC */],
    ["urn:cen.eu:en16931:2017#compliant#urn:zugferd.de:2p0:basic", "BASIC" /* BASIC */],
...
function toXRechnung(input, options) {
  ...
  let validation;
  if (validate) {
    validation = validateInput(enrichedInput, profile, "xrechnung" /* XRECHNUNG */);
    if (!validation.valid) {
      throw new Error(
...
export { DocumentTypeCode, FLAVOR_CONFIGS, FLAVOR_DEFAULT_TYPE_CODES, Flavor, PROFILE_MAIN_XSD, PROFILE_SCHEMA_DIRS, PROFILE_URNS, Profile, UnitCode, VatCategoryCode, XRECHNUNG_PROFILE_URN, buildXml, embedFacturX, escapeXml, extractXml, getFlavorConfig, resolveBusinessProcessUrn, resolveTypeCode, toXRechnung, validateFlavorProfile, validateInput, validateXsd };
